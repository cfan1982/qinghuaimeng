  ...
  const addJustifiedGallery = async (elements, tabs = false) => {
    ...
    elements.forEach(element => {
      ...
      if (element.classList.contains('no-gallery') || 
          element.getAttribute('data-no-gallery') === 'true') {
        return
      }
      ...
      try {
        ...
        // 处理容器
        const container = element.querySelector('.gallery-container')
        if (container) {
          container.style.visibility = 'visible'
          container.style.display = 'grid'
        }
        ...
        // 处理所有 fancybox
        const fancyboxes = element.querySelectorAll('.fancybox')
        ...
        // 移除加载状态
        btf.setLoading.remove(element)
        element.classList.add('loaded')
      } catch (error) {
        console.warn('Failed to process gallery element:', error)
      }
    })
  }
